...
const SoundButton = ({ arabic }) => {
    ...
    useEffect(() => {
        const checkAudio = async () => {
            try {
                const response = await fetch(`/audio/numbers/${arabic}.mp3`, { method: 'HEAD' });
                ...
                <div className="kana-table">
                    <div className="table-numbers-row">
                        {filteredData.map((item, idx) => (
                            <div className="table-numbers-cell" key={idx}>
                                <div>{item.arabic}</div>
                                <div>{item.kanji}</div>
                                <div>{item.question}</div>
                                <div>{item.reading}</div>
                                <div><SoundButton arabic={item.arabic} /></div>
                            </div>
                        ))}
                    </div>
                </div>
