// Безопасный менеджер статистики с правильной инициализацией AsyncStorage
...
// Безопасная работа с AsyncStorage
const safeAsyncStorage = {
  getItem: async (key) => {
    try {
      const asyncStorage = getAsyncStorage();
      if (!asyncStorage) {
        console.warn('AsyncStorage not available for getItem');
        return null;
      }
      return await asyncStorage.getItem(key);
    } catch (error) {
      console.error('Error in safeAsyncStorage.getItem:', error);
      return null;
    }
  },
  ...
  setItem: async (key, value) => {
    try {
      const asyncStorage = getAsyncStorage();
      if (!asyncStorage) {
        console.warn('AsyncStorage not available for setItem');
        return false;
      }
      await asyncStorage.setItem(key, value);
      return true;
    } catch (error) {
      console.error('Error in safeAsyncStorage.setItem:', error);
      return false;
    }
  },
  ...
  removeItem: async (key) => {
    try {
      const asyncStorage = getAsyncStorage();
      if (!asyncStorage) {
        console.warn('AsyncStorage not available for removeItem');
        return false;
      }
      await asyncStorage.removeItem(key);
      return true;
    } catch (error) {
      console.error('Error in safeAsyncStorage.removeItem:', error);
      return false;
    }
  },
  ...
  multiRemove: async (keys) => {
    try {
      const asyncStorage = getAsyncStorage();
      if (!asyncStorage) {
        console.warn('AsyncStorage not available for multiRemove');
        return false;
      }
      await asyncStorage.multiRemove(keys);
      return true;
    } catch (error) {
      console.error('Error in safeAsyncStorage.multiRemove:', error);
      return false;
    }
  }
};
...
export const loadStats = async () => {
  try {
    if (!isAsyncStorageInitialized()) {
      console.log('Using fallback storage - AsyncStorage not initialized');
      return fallbackStorage.userStats || getDefaultStats();
    }

    const storedStats = await safeAsyncStorage.getItem(STORAGE_KEYS.USER_STATS);
    if (storedStats) {
...
export const saveStats = async (stats) => {
  try {
    ...
    const success = await safeAsyncStorage.setItem(STORAGE_KEYS.USER_STATS, JSON.stringify(stats));
    if (success) {
      console.log('Stats saved to AsyncStorage');
    } else {
      console.log('Failed to save to AsyncStorage, using fallback');
      fallbackStorage.userStats = stats;
    }
  } catch (error) {
...
export const getSymbolStats = async () => {
  try {
    ...
    const storedStats = await safeAsyncStorage.getItem(STORAGE_KEYS.SYMBOL_STATS);
    if (storedStats) {
      const parsedStats = JSON.parse(storedStats);
      if (parsedStats && typeof parsedStats === 'object') {
        return parsedStats;
...
const saveSymbolStats = async (symbolStats) => {
  try {
    ...
    const success = await safeAsyncStorage.setItem(STORAGE_KEYS.SYMBOL_STATS, JSON.stringify(symbolStats));
    if (!success) {
      fallbackStorage.symbolStats = symbolStats;
    }
  } catch (error) {
...
export const getDailyStats = async (date = new Date()) => {
  try {
    ...
    const dateKey = date.toISOString().split('T')[0];
    const storedDailyStats = await safeAsyncStorage.getItem(STORAGE_KEYS.DAILY_STATS);
...
const saveDailyStats = async (dailyStats) => {
  try {
    ...
    const success = await safeAsyncStorage.setItem(STORAGE_KEYS.DAILY_STATS, JSON.stringify(dailyStats));
    if (!success) {
      fallbackStorage.dailyStats = dailyStats;
    }
  } catch (error) {
...
export const exportStats = async () => {
  try {
    ...
    const exportData = {
      ...
      version: '1.0',
      storageType: isAsyncStorageInitialized() ? 'asyncstorage' : 'memory',
    };
