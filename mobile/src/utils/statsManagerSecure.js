// Менеджер статистики с использованием Expo SecureStore
// Более надежная альтернатива AsyncStorage
...
// Fallback storage для случаев, когда SecureStore недоступен
let fallbackStorage = {
  userStats: null,
  symbolStats: {},
  dailyStats: {},
...
// Безопасная работа с SecureStore
const safeSecureStore = {
  getItem: async (key) => {
    try {
      if (!isSecureStoreAvailable()) {
        console.warn('SecureStore not available for getItem');
        return null;
      }
      return await SecureStore.getItemAsync(key);
    } catch (error) {
      console.error('Error in safeSecureStore.getItem:', error);
      return null;
    }
  },
  ...
  setItem: async (key, value) => {
    try {
      if (!isSecureStoreAvailable()) {
        console.warn('SecureStore not available for setItem');
        return false;
      }
      await SecureStore.setItemAsync(key, value);
      return true;
    } catch (error) {
      console.error('Error in safeSecureStore.setItem:', error);
      return false;
    }
  },
  ...
  deleteItem: async (key) => {
    try {
      if (!isSecureStoreAvailable()) {
        console.warn('SecureStore not available for deleteItem');
        return false;
      }
      await SecureStore.deleteItemAsync(key);
      return true;
    } catch (error) {
      console.error('Error in safeSecureStore.deleteItem:', error);
      return false;
    }
  }
};
...
export const loadStats = async () => {
  try {
    if (!isSecureStoreAvailable()) {
      console.log('Using fallback storage - SecureStore not available');
      return fallbackStorage.userStats || getDefaultStats();
    }

    const storedStats = await safeSecureStore.getItem(STORAGE_KEYS.USER_STATS);
    if (storedStats) {
...
export const saveStats = async (stats) => {
  try {
    ...
    const success = await safeSecureStore.setItem(STORAGE_KEYS.USER_STATS, JSON.stringify(stats));
    if (success) {
      console.log('Stats saved to SecureStore');
    } else {
      console.log('Failed to save to SecureStore, using fallback');
      fallbackStorage.userStats = stats;
    }
  } catch (error) {
...
export const getSymbolStats = async () => {
  try {
    if (!isSecureStoreAvailable()) {
      return fallbackStorage.symbolStats || {};
    }

    const storedStats = await safeSecureStore.getItem(STORAGE_KEYS.SYMBOL_STATS);
    if (storedStats) {
      const parsedStats = JSON.parse(storedStats);
      if (parsedStats && typeof parsedStats === 'object') {
        return parsedStats;
...
const saveSymbolStats = async (symbolStats) => {
  try {
    if (!isSecureStoreAvailable()) {
      fallbackStorage.symbolStats = symbolStats;
      return;
    }

    const success = await safeSecureStore.setItem(STORAGE_KEYS.SYMBOL_STATS, JSON.stringify(symbolStats));
    if (!success) {
      fallbackStorage.symbolStats = symbolStats;
    }
  } catch (error) {
...
export const clearAllStats = async () => {
  try {
    if (isSecureStoreAvailable()) {
      await safeSecureStore.deleteItem(STORAGE_KEYS.USER_STATS);
      await safeSecureStore.deleteItem(STORAGE_KEYS.SYMBOL_STATS);
      await safeSecureStore.deleteItem(STORAGE_KEYS.DAILY_STATS);
    }
...
export const getDailyStats = async (date = new Date()) => {
  try {
    if (!isSecureStoreAvailable()) {
      ...
      const dailyStats = fallbackStorage.dailyStats || {};
    ...
    const dateKey = date.toISOString().split('T')[0];
    const storedDailyStats = await safeSecureStore.getItem(STORAGE_KEYS.DAILY_STATS);
...
const saveDailyStats = async (dailyStats) => {
  try {
    if (!isSecureStoreAvailable()) {
      fallbackStorage.dailyStats = dailyStats;
      return;
    }

    const success = await safeSecureStore.setItem(STORAGE_KEYS.DAILY_STATS, JSON.stringify(dailyStats));
    if (!success) {
      fallbackStorage.dailyStats = dailyStats;
    }
  } catch (error) {
...
export const exportStats = async () => {
  try {
    ...
    const exportData = {
      ...
      version: '1.0',
      storageType: isSecureStoreAvailable() ? 'securestore' : 'memory',
    };
