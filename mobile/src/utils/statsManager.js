// Менеджер статистики с сохранением в AsyncStorage
// Для совместимости с Expo и сохранения данных между сессиями

import { getAsyncStorage, isAsyncStorageInitialized } from './asyncStorageInit';

// Fallback storage для случаев, когда AsyncStorage недоступен
let fallbackStorage = {
  userStats: null,
  symbolStats: {},
  dailyStats: {},
};

// Принудительный fallback режим (для отладки)
const FORCE_FALLBACK = false; // Установите true для принудительного использования fallback

// Проверка доступности AsyncStorage
const isAsyncStorageAvailable = () => {
  try {
    // Используем безопасную проверку инициализации
    return isAsyncStorageInitialized();
  } catch (error) {
    console.error('Error checking AsyncStorage availability:', error);
    return false;
  }
};

// Ключи для AsyncStorage
const STORAGE_KEYS = {
  USER_STATS: 'user_stats',
  SYMBOL_STATS: 'symbol_stats',
  DAILY_STATS: 'daily_stats',
};

// Инициализация статистики по умолчанию
const getDefaultStats = () => ({
  totalTests: 0,
  totalCorrect: 0,
  totalIncorrect: 0,
  totalTime: 0,
  currentStreak: 0,
  bestStreak: 0,
  lastTestDate: null,
  quizStats: {},
});

// Загрузка статистики из AsyncStorage
export const loadStats = async () => {
  try {
    // Принудительный fallback режим или проверка доступности
    if (FORCE_FALLBACK || !isAsyncStorageAvailable()) {
      console.warn('Using fallback storage (forced or AsyncStorage unavailable)');
      return fallbackStorage.userStats || getDefaultStats();
    }

    const asyncStorage = getAsyncStorage();
    if (!asyncStorage) {
      console.warn('AsyncStorage not available, using fallback');
      return fallbackStorage.userStats || getDefaultStats();
    }

    const storedStats = await asyncStorage.getItem(STORAGE_KEYS.USER_STATS);
    if (storedStats) {
      const parsedStats = JSON.parse(storedStats);
      // Проверяем, что загруженные данные имеют правильную структуру
      if (parsedStats && typeof parsedStats === 'object') {
        return { ...getDefaultStats(), ...parsedStats };
      }
    }
    return getDefaultStats();
  } catch (error) {
    console.error('Error loading stats from AsyncStorage:', error);
    return fallbackStorage.userStats || getDefaultStats();
  }
};

// Сохранение статистики в AsyncStorage
export const saveStats = async (stats) => {
  try {
    if (stats === null || stats === undefined) {
      console.warn('Attempting to save null/undefined stats');
      return;
    }

    // Принудительный fallback режим или проверка доступности
    if (FORCE_FALLBACK || !isAsyncStorageAvailable()) {
      console.warn('Using fallback storage (forced or AsyncStorage unavailable)');
      fallbackStorage.userStats = stats;
      return;
    }
    
    const statsToSave = JSON.stringify(stats);
    await AsyncStorage.setItem(STORAGE_KEYS.USER_STATS, statsToSave);
    console.log('Stats saved to AsyncStorage:', stats);
  } catch (error) {
    console.error('Error saving stats to AsyncStorage:', error);
    // Fallback на in-memory storage при ошибке
    fallbackStorage.userStats = stats;
  }
};

// Обновление статистики после теста
export const updateStatsAfterTest = async (testResults) => {
  try {
    if (!testResults) {
      console.warn('Invalid test results');
      return null;
    }
    
    const currentStats = await loadStats();
    const updatedStats = calculateNewStats(currentStats, testResults);
    await saveStats(updatedStats);
    return updatedStats;
  } catch (error) {
    console.error('Error updating stats:', error);
    return null;
  }
};

// Получение статистики по символам из AsyncStorage
export const getSymbolStats = async () => {
  try {
    if (!isAsyncStorageAvailable()) {
      console.warn('AsyncStorage is not available, returning empty symbol stats');
      return {};
    }

    const storedSymbolStats = await AsyncStorage.getItem(STORAGE_KEYS.SYMBOL_STATS);
    if (storedSymbolStats) {
      const parsedStats = JSON.parse(storedSymbolStats);
      if (parsedStats && typeof parsedStats === 'object') {
        return parsedStats;
      }
    }
    return {};
  } catch (error) {
    console.error('Error loading symbol stats from AsyncStorage:', error);
    return {};
  }
};

// Сохранение статистики по символам в AsyncStorage
const saveSymbolStats = async (symbolStats) => {
  try {
    if (!isAsyncStorageAvailable()) {
      console.warn('AsyncStorage is not available, cannot save symbol stats');
      return;
    }

    const statsToSave = JSON.stringify(symbolStats);
    await AsyncStorage.setItem(STORAGE_KEYS.SYMBOL_STATS, statsToSave);
  } catch (error) {
    console.error('Error saving symbol stats to AsyncStorage:', error);
  }
};

// Обновление статистики по символам
export const updateSymbolStats = async (symbol, isCorrect) => {
  try {
    if (!symbol) {
      console.warn('Invalid symbol');
      return null;
    }
    
    const symbolStats = await getSymbolStats();
    
    if (!symbolStats[symbol]) {
      symbolStats[symbol] = {
        correct: 0,
        incorrect: 0,
        lastSeen: new Date().toISOString(),
        streak: 0,
        bestStreak: 0,
      };
    }

    const symbolStat = symbolStats[symbol];
    
    if (isCorrect) {
      symbolStat.correct += 1;
      symbolStat.streak += 1;
      if (symbolStat.streak > symbolStat.bestStreak) {
        symbolStat.bestStreak = symbolStat.streak;
      }
    } else {
      symbolStat.incorrect += 1;
      symbolStat.streak = 0;
    }

    symbolStat.lastSeen = new Date().toISOString();
    
    await saveSymbolStats(symbolStats);
    console.log('Symbol stats updated:', symbol, symbolStat);
    return symbolStats;
  } catch (error) {
    console.error('Error updating symbol stats:', error);
    return null;
  }
};

// Очистка всей статистики
export const clearAllStats = async () => {
  try {
    if (!isAsyncStorageAvailable()) {
      console.warn('AsyncStorage is not available, cannot clear stats');
      return;
    }

    await AsyncStorage.multiRemove([
      STORAGE_KEYS.USER_STATS,
      STORAGE_KEYS.SYMBOL_STATS,
      STORAGE_KEYS.DAILY_STATS,
    ]);
    console.log('All stats cleared from AsyncStorage');
  } catch (error) {
    console.error('Error clearing stats from AsyncStorage:', error);
  }
};

// Получение статистики за день из AsyncStorage
export const getDailyStats = async (date = new Date()) => {
  try {
    if (!isAsyncStorageAvailable()) {
      console.warn('AsyncStorage is not available, returning default daily stats');
      return {
        tests: 0,
        correct: 0,
        incorrect: 0,
        time: 0,
        accuracy: 0,
      };
    }

    const dateKey = date.toISOString().split('T')[0];
    const storedDailyStats = await AsyncStorage.getItem(STORAGE_KEYS.DAILY_STATS);
    
    if (storedDailyStats) {
      const dailyStats = JSON.parse(storedDailyStats);
      if (dailyStats && typeof dailyStats === 'object') {
        return dailyStats[dateKey] || {
          tests: 0,
          correct: 0,
          incorrect: 0,
          time: 0,
          accuracy: 0,
        };
      }
    }
    
    return {
      tests: 0,
      correct: 0,
      incorrect: 0,
      time: 0,
      accuracy: 0,
    };
  } catch (error) {
    console.error('Error loading daily stats from AsyncStorage:', error);
    return {
      tests: 0,
      correct: 0,
      incorrect: 0,
      time: 0,
      accuracy: 0,
    };
  }
};

// Сохранение дневной статистики в AsyncStorage
const saveDailyStats = async (dailyStats) => {
  try {
    if (!isAsyncStorageAvailable()) {
      console.warn('AsyncStorage is not available, cannot save daily stats');
      return;
    }

    const statsToSave = JSON.stringify(dailyStats);
    await AsyncStorage.setItem(STORAGE_KEYS.DAILY_STATS, statsToSave);
  } catch (error) {
    console.error('Error saving daily stats to AsyncStorage:', error);
  }
};

// Обновление дневной статистики
export const updateDailyStats = async (testResults) => {
  try {
    if (!testResults) {
      console.warn('Invalid test results');
      return null;
    }
    
    const dateKey = new Date().toISOString().split('T')[0];
    const storedDailyStats = await AsyncStorage.getItem(STORAGE_KEYS.DAILY_STATS);
    const dailyStats = storedDailyStats ? JSON.parse(storedDailyStats) : {};
    
    if (!dailyStats[dateKey]) {
      dailyStats[dateKey] = {
        tests: 0,
        correct: 0,
        incorrect: 0,
        time: 0,
        accuracy: 0,
      };
    }

    const dayStats = dailyStats[dateKey];
    dayStats.tests += 1;
    dayStats.correct += testResults.correct;
    dayStats.incorrect += testResults.incorrect;
    dayStats.time += testResults.timeElapsed;
    dayStats.accuracy = Math.round((dayStats.correct / (dayStats.correct + dayStats.incorrect)) * 100);

    await saveDailyStats(dailyStats);
    console.log('Daily stats updated:', dateKey, dayStats);
    return dailyStats;
  } catch (error) {
    console.error('Error updating daily stats:', error);
    return null;
  }
};

// Экспорт статистики
export const exportStats = async () => {
  try {
    const userStats = await loadStats();
    const symbolStats = await getSymbolStats();
    const storedDailyStats = await AsyncStorage.getItem(STORAGE_KEYS.DAILY_STATS);
    const dailyStats = storedDailyStats ? JSON.parse(storedDailyStats) : {};
    
    const exportData = {
      userStats,
      symbolStats,
      dailyStats,
      exportDate: new Date().toISOString(),
      version: '1.0',
    };

    console.log('Stats exported:', exportData);
    return exportData;
  } catch (error) {
    console.error('Error exporting stats:', error);
    return null;
  }
};

// Импорт статистики
export const importStats = async (importData) => {
  try {
    if (importData.userStats) {
      await saveStats(importData.userStats);
    }
    if (importData.symbolStats) {
      await saveSymbolStats(importData.symbolStats);
    }
    if (importData.dailyStats) {
      await saveDailyStats(importData.dailyStats);
    }
    console.log('Stats imported:', importData);
    return true;
  } catch (error) {
    console.error('Error importing stats:', error);
    return false;
  }
};

// Вспомогательные функции
const calculateNewStats = (currentStats, testResults) => {
  const {
    correct,
    incorrect,
    timeElapsed,
    quizType,
    accuracy,
  } = testResults;

  // Обновляем общую статистику
  const newStats = {
    ...currentStats,
    totalTests: currentStats.totalTests + 1,
    totalCorrect: currentStats.totalCorrect + correct,
    totalIncorrect: currentStats.totalIncorrect + incorrect,
    totalTime: currentStats.totalTime + timeElapsed,
    lastTestDate: new Date().toISOString(),
  };

  // Обновляем статистику по типам тестов
  if (quizType) {
    if (!newStats.quizStats[quizType]) {
      newStats.quizStats[quizType] = {
        tests: 0,
        correct: 0,
        incorrect: 0,
        accuracy: 0,
      };
    }

    const quizStat = newStats.quizStats[quizType];
    quizStat.tests += 1;
    quizStat.correct += correct;
    quizStat.incorrect += incorrect;
    quizStat.accuracy = Math.round((quizStat.correct / (quizStat.correct + quizStat.incorrect)) * 100);
  }

  // Обновляем серию дней - исправленная логика
  const newStreak = calculateStreak(currentStats.lastTestDate, currentStats.currentStreak);
  newStats.currentStreak = newStreak;
  if (newStreak > currentStats.bestStreak) {
    newStats.bestStreak = newStreak;
  }

  return newStats;
};

const calculateStreak = (lastTestDate, currentStreak = 0) => {
  if (!lastTestDate) return 1;

  const lastDate = new Date(lastTestDate);
  const today = new Date();
  const yesterday = new Date(today);
  yesterday.setDate(yesterday.getDate() - 1);

  // Если последний тест был вчера, увеличиваем серию
  if (lastDate.toDateString() === yesterday.toDateString()) {
    return currentStreak + 1;
  }
  // Если последний тест был сегодня, сохраняем текущую серию
  else if (lastDate.toDateString() === today.toDateString()) {
    return currentStreak;
  }
  // Если последний тест был раньше, сбрасываем серию
  else {
    return 1; // Начинаем новую серию
  }
};
